...
class TestYoutubeGenerator(unittest.TestCase):
    def test_directory_and_file_creation(self):
        command = "python3 generators/youtube_generator.py 10 -o ./testplans -c 19 -l en"
        subprocess.run(command, shell=True)

        target_directory = "./testplans"
        self.assertTrue(os.path.exists(target_directory), f"{target_directory} does not exist.")
        time_str = time.strftime("%Y%m%d-%H%M%S")

        filename = f"Travel&Events_en_CAUQAQ_{time_str}.json"
        
        json_file_path = os.path.join(target_directory, filename)  
        self.assertTrue(os.path.exists(json_file_path), f"{json_file_path} does not exist.")
    ...
    def test_missing_required_parameter(self):
        ...
        self.assertIn(expected_error_message, result.stderr.decode(), f"Expected error message not found in stderr.\nActual stderr: {result.stderr.decode()}")
    ...
    def test_testplan_execute_test_model(self):
        ...
        command = f"python3 youtube_runner.py './testplans/{filename}' -st DummyTest -m tiny"
        subprocess.run(command, shell=True)

        filename = f"YouTubeTestRunner_Travel&Events_{time_str}.json"
        output_directory = "./output"
        output_path = os.path.join(output_directory, filename)
        self.assertTrue(os.path.exists(output_path), f"Output file {output_path} not found.")
    ...
    def test_execute_with_missing_parameter(self):
        ...
        self.assertIn(expected_error_message, result.stderr, f"Expected error message not found in stderr.\nActual stderr: {result.stderr.decode()}")
