...
class TestDummyTest(unittest.TestCase):
    ...
    @mock.patch("src.differs.jiwer_differ")
    @mock.patch(
        "jiwer.compute_measures", side_effect=lambda x, y: {"wer": 0.1, "mer": 0.2}
    )
    ...
    @mock.patch(
        "rapidfuzz.distance.Levenshtein.editops", side_effect=NotImplementedError
    )
    def test_compare_invalid_paths(
        self, mock_editops, mock_compute_measures, mock_differ
    ):
        model_transcript = "Model Transcript"
